...
In this operator, we **pin** Sigstore trusted root material (either embedded `internal/security/trusted_root.json` or a user-provided ConfigMap) and pass it to Cosign for both keyless verification and Rekor transparency log verification. This avoids fetching/updating trusted root data via TUF at runtime. The only TUF fetch logic in this repository is `internal/security/fetch_trusted_root.go`, which is build-ignored and intended for maintainers to refresh the pinned `trusted_root.json`.
