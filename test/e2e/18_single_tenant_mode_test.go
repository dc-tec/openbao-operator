...
// The single-tenant mode test validates that:
// 1. The operator can run with WATCH_NAMESPACE set
// 2. Event-driven reconciliation works (via Owns() watches)
// 3. No Provisioner/OpenBaoTenant is needed
var _ = Describe("Single-Tenant Mode", Label("single-tenant", "tenancy"), Ordered, func() {
