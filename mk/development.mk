...
verify-openbao-config-compat: ## Validate generated HCL fixtures against upstream OpenBao config parser (semantic).
	@bash hack/ci/openbao-config-compat.sh 2.4.4 2.5.5 2.6.1
...
report-openbao-config-schema-drift: ## Report upstream OpenBao config schema drift across the supported range (non-failing).
	@REPORT_SCHEMA_DRIFT=true bash hack/ci/openbao-config-compat.sh 2.4.4 2.5.5 2.6.1
...
report-openbao-operator-schema-drift: ## Report operator-vs-upstream OpenBao config schema drift (non-failing).
	@GOFLAGS="-mod=mod" go run ./hack/tools/openbao_operator_schema_drift --openbao-image-tag 2.6.1
...
PERF_OPENBAO_VERSION ?= 2.6.1
PERF_OPENBAO_IMAGE ?= openbao/openbao:2.6.1
PERF_UPGRADE_FROM_VERSION ?= 2.6.0
PERF_UPGRADE_FROM_IMAGE ?= openbao/openbao:2.6.0
PERF_UPGRADE_TO_VERSION ?= 2.6.1
PERF_UPGRADE_TO_IMAGE ?= openbao/openbao:2.6.1
