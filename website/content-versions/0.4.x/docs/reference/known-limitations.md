...
| Voter resources | The API has no top-level voter CPU or memory requests and limits; only read replicas expose `template.resources` | Enforce an external policy or account for the current scheduling gap |
