...
| Gateway exposure | `GatewayIntegrationReady`; inspect Route conditions separately |
...
| `GatewayIntegrationReady` | Referenced Gateway and GatewayClass prerequisites; not Route attachment |
...
Use `status.provisioned` as the success signal and inspect `status.lastError` when it is false. The controller records
the `Provisioned` condition as false with reason `SecurityViolation` for a forbidden cross-namespace target, but it
does not currently write that condition as true after a successful reconciliation.
