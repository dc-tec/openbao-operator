...
| `GatewayIntegrationReady=False` | Gateway and GatewayClass prerequisites; inspect Route status separately |
...
For Gateway API, TLS termination in front of an ACME listener is not equivalent to TLS passthrough. Check the actual
Route attachment and backend policy, not only `GatewayIntegrationReady`.
