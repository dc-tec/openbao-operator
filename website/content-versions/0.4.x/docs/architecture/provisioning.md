...
the target namespace so cluster finalizers can still run. After the last cluster is gone, it deletes the core tenant Role
and RoleBinding and both Secret allowlist Role and RoleBinding pairs, then removes the tenant finalizer.

Current cleanup is intentionally described narrowly: it does not delete the provisioned `ResourceQuota` or `LimitRange`,
and it does not restore namespace Pod Security labels. Platform owners must account for those retained namespace-level
policies when deprovisioning a tenant.
