...
{{< callout type="warning" title="Provisioner metrics have a chart NetworkPolicy gap" >}}
In multi-tenant mode, the current Helm NetworkPolicy admits scraper traffic to controller pods but not provisioner
pods. The provisioner ServiceMonitor is rendered, yet its target remains unreachable while that policy is enforced.
Treat provisioner metrics as unavailable through the chart default until the selector contract is fixed or your
platform supplies an explicit, reviewed policy for that pod.
{{< /callout >}}
...
When the chart NetworkPolicy is enabled, the monitoring namespace must match
`networkPolicy.metricsAllowedNamespaceLabels`, which defaults to `metrics: enabled`.
