...
## Configure read-replica storage and resources
...
The current API exposes CPU and memory requests and limits only for read replicas:

{{< command label="configure" title="Size the read-replica pool" >}}
spec:
  readReplicas:
    replicas: 2
    template:
...
{{< callout type="warning" title="Voter resource sizing is not exposed" >}}
There is no top-level `spec.resources` field in the current CRD. Generated voter containers have no configurable
resource requests or limits. `spec.readReplicas.template.resources` applies only to read replicas. Treat voter sizing as
a product gap when evaluating production admission, quota, scheduling, and noisy-neighbor requirements.
{{< /callout >}}

Do not copy manifests that use top-level `spec.resources`; strict CRD decoding rejects that field.
