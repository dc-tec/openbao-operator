...
Use Helm multi-tenant mode for the core guide. The current Helm single-tenant settings do not set the runtime
`WATCH_NAMESPACE`; the dedicated guide uses the tested Kustomize overlay instead.
