...
description: Install OpenBao Operator 0.4.2 with the intended tenancy, platform, identity, CRDs, and admission policy contract.
...
Install a pinned 0.4.2 release and verify the rendered namespace, identities, controllers, CRDs, and admission policies.
...
| Dedicated single namespace | `config/overlays/single-tenant` | Controller-only runtime and `WATCH_NAMESPACE` |
...
2. Inspect the chart defaults when the platform needs overrides.
   ...
   [`values.yaml`](https://github.com/dc-tec/openbao-operator/blob/0.4.2/charts/openbao-operator/values.yaml).
...
{{< command label="apply" title="Apply the 0.4.2 installer manifest" >}}
kubectl apply -f \
  https://github.com/dc-tec/openbao-operator/releases/download/0.4.2/install.yaml
{{< /command >}}
...
{{< command label="upgrade" title="Upgrade to 0.4.2" >}}
kubectl apply -f \
