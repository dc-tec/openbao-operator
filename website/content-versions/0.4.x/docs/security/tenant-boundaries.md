...
namespace is gone, then removes the controller RoleBinding, tenant Role, and Secret allowlist RBAC.

Tenant cleanup does not remove the ResourceQuota, LimitRange, or Pod Security labels. Review those resources
explicitly before reusing or deleting the namespace. Cluster-owned NetworkPolicies follow the cluster lifecycle, not
the tenant finalizer.
