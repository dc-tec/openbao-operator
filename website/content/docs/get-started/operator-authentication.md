...
Self-init runs once. By default, an administrator applies later policy changes required by an operator upgrade.
Opt-in [policy reconciliation](../operator-authorization/) can maintain approved operational policies. It does not
update JWT roles or their accepted audiences.
