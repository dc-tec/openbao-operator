...
# PERF_NODE_IMAGE pins the Kind node image used for performance baseline/gate runs.
PERF_NODE_IMAGE ?= kindest/node:v1.34.3
# PERF_RUNS controls baseline sample size per scenario.
PERF_RUNS ?= 5
# PERF_SCENARIO_TIMEOUT is the timeout per performance scenario execution.
PERF_SCENARIO_TIMEOUT ?= 90m
# PERF_SMOKE_SCENARIO_TIMEOUT is the timeout for the PR smoke performance gate.
PERF_SMOKE_SCENARIO_TIMEOUT ?= 45m
# PERF_SMOKE_SCENARIOS are the scenarios executed in the PR smoke performance gate.
PERF_SMOKE_SCENARIOS ?= lifecycle
# PERF_BASELINE_OUT is the baseline evidence JSON path generated by perf-baseline.
