...
func TestSetGatewayIntegrationReadyEvaluatedCondition_AllowsKnownReasonStatusPairs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "gateway route not accepted", status: metav1.ConditionFalse, reason: ReasonGatewayRouteNotAccepted},
		{name: "gateway route references unresolved", status: metav1.ConditionFalse, reason: ReasonGatewayRouteReferencesUnresolved},
		...
		{name: "gateway route pending", status: metav1.ConditionUnknown, reason: ReasonGatewayRoutePending},
