...
func TestMapClaimToUpgradeRequests(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	scheme := newTestScheme(t)
	reconciler := &OpenBaoClusterClaimUpgradeRequestReconciler{
	...
	if requests[0].NamespacedName != (client.ObjectKey{Namespace: "payments", Name: "upgrade-1"}) {
		t.Fatalf("first request key = %#v, want payments/upgrade-1", requests[0].NamespacedName)
	}
	if requests[1].NamespacedName != (client.ObjectKey{Namespace: "payments", Name: "upgrade-2"}) {
		t.Fatalf("second request key = %#v, want payments/upgrade-2", requests[1].NamespacedName)
	}
}

func TestMapClaimManagedClusterToUpgradeRequests(t *testing.T) {
	t.Parallel()
	...
	t.Run("maps claim managed cluster by ownership labels", func(t *testing.T) {
		...
		if requests[0].NamespacedName != (client.ObjectKey{Namespace: "payments", Name: "upgrade-1"}) {
			t.Fatalf("request key = %#v, want payments/upgrade-1", requests[0].NamespacedName)
		}
	})
...
func TestReconcile(t *testing.T) {
	...
	t.Run("requires app reconciler", func(t *testing.T) {
		...
		if err.Error() != "openbaoclusterclaimupgraderequest app reconciler is not configured" {
	...
	t.Run("delegates to app reconciler", func(t *testing.T) {
		...
		reconciler := &OpenBaoClusterClaimUpgradeRequestReconciler{AppReconciler: app}
		req := ctrl.Request{NamespacedName: types.NamespacedName{Namespace: "payments", Name: "upgrade-1"}}
