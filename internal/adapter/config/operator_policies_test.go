		...
		t.Run(string(strategy), func(t *testing.T) {
			...
			var approval struct {
				Paths []struct {
					...
					Required     []string            `hcl:"required_parameters"`
			...
			for i, policy := range OperatorPolicies(cluster) {
				...
				require.Equal(t, []string{"update"}, rule.Capabilities)
				require.Equal(t, []string{"policy"}, rule.Required)
...
func TestPolicyApprovalBootstrapIsOptIn(t *testing.T) {
	...
	for _, enabled := range []bool{false, true} {
		cluster.Spec.SelfInit.OIDC.ReconcilePolicies = enabled
