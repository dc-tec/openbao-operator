...
//   - Managing a per-cluster static auto-unseal Secret.
//   - Rendering a config.hcl ConfigMap that injects TLS paths, storage configuration, retry_join, and static seal.
...
func (m *Manager) Reconcile(ctx context.Context, logger logr.Logger, cluster *openbaov1alpha1.OpenBaoCluster) error {
	...
	if err := m.ensureStatefulSet(ctx, logger, cluster, configContent); err != nil {
		return err
	}
