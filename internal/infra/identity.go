...
// checkKubernetesAuthRBAC verifies that the required ClusterRoleBinding exists for
// Kubernetes Auth to work. The operator does NOT create this automatically due to
// security and compliance considerations (cluster-scoped RBAC often requires manual
// approval). Instead, it checks if it exists and logs a warning if missing.
//
// Security Note: The system:auth-delegator ClusterRole grants cluster-wide permission
// to verify ServiceAccount tokens via the TokenReview API. This is necessary because
// TokenReview is a cluster-scoped API endpoint. The ServiceAccount can verify tokens
// but cannot impersonate them or use them to access resources. This is the standard
// pattern for OpenBao/Vault Kubernetes Auth.
//
// Users must manually create the ClusterRoleBinding.
func (m *Manager) checkKubernetesAuthRBAC(ctx context.Context, logger logr.Logger, cluster *openbaov1alpha1.OpenBaoCluster) error {
	// Check if Kubernetes Auth is enabled in OpenBao (via selfInit)
	kubernetesAuthEnabled := false
	if cluster.Spec.SelfInit != nil && cluster.Spec.SelfInit.Enabled && cluster.Spec.SelfInit.Requests != nil {
		for _, req := range cluster.Spec.SelfInit.Requests {
			if req.Path == "sys/auth/kubernetes" {
	...
	// Also check if backups are configured to use Kubernetes Auth
	backupsUseKubernetesAuth := cluster.Spec.Backup != nil && cluster.Spec.Backup.KubernetesAuthRole != ""

	// Only check if Kubernetes Auth is actually being used
	if !kubernetesAuthEnabled && !backupsUseKubernetesAuth {
		return nil
	}

	saName := serviceAccountName(cluster)
	clusterRoleBindingName := cluster.Name + "-kubernetes-auth"
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			logger.Info(
				"ClusterRoleBinding for Kubernetes Auth not found; Kubernetes Auth will not work until it is created",
				"clusterrolebinding", clusterRoleBindingName,
				"serviceaccount", saName,
				"namespace", cluster.Namespace,
				"hint", "See test/cluster/openbao-kubernetes-auth-rbac.yaml for an example",
			)
			return nil // Don't fail reconciliation, just warn
		}
		return fmt.Errorf("failed to check ClusterRoleBinding %s: %w", clusterRoleBindingName, err)
	}

	// Verify the binding is correctly configured
	if clusterRoleBinding.RoleRef.Name != "system:auth-delegator" {
		logger.Info(
			"ClusterRoleBinding exists but references incorrect ClusterRole",
			"clusterrolebinding", clusterRoleBindingName,
			"expected", "system:auth-delegator",
			"actual", clusterRoleBinding.RoleRef.Name,
		)
	}

	// Check if the ServiceAccount is in the subjects
	found := false
	...
	if !found {
		logger.Info(
			"ClusterRoleBinding exists but does not include the required ServiceAccount",
			"clusterrolebinding", clusterRoleBindingName,
			"serviceaccount", saName,
			"namespace", cluster.Namespace,
		)
