...
func TestDefaultBackupImage_WithVersion(t *testing.T) {
	...
	t.Setenv(EnvOperatorVersion, "1.2.3")
	...
	want := DefaultBackupImageRepository + ":1.2.3"
	if got != want {
...
func TestDefaultBackupImage_WithCustomRepo(t *testing.T) {
	...
	want := "my-registry.io/custom-backup:1.2.3"
	if got != want {
		t.Errorf("DefaultBackupImage() = %v, want %v", got, want)
	}
}
...
func TestDefaultUpgradeImage_WithVersion(t *testing.T) {
	t.Setenv(EnvOperatorVersion, "2.0.0")
	...
	want := DefaultUpgradeImageRepository + ":2.0.0"
	if got != want {
...
func TestDefaultInitImage_WithVersion(t *testing.T) {
	t.Setenv(EnvOperatorVersion, "3.0.0")
	...
	want := DefaultInitImageRepository + ":3.0.0"
	if got != want {
