...
func (r runtimeReconciler) Reconcile(ctx context.Context, key types.NamespacedName, logger logr.Logger) (recon.Result, error) {
	...
	r.emitBackupRequestEvent(original, request)

	logger.Info("Reconciled OpenBaoClusterClaimBackupRequest", "state", evaluation.state, "reason", evaluation.reason)
	return recon.Result{}, nil
}

func (r runtimeReconciler) emitBackupRequestEvent(
	original *openbaov1alpha1.OpenBaoClusterClaimBackupRequest,
	request *openbaov1alpha1.OpenBaoClusterClaimBackupRequest,
) {
	...
	note := fmt.Sprintf("Backup request for claim %s is %s", request.Spec.ClaimRef.Name, request.Status.State)
	...
	requestworkflow.EmitEvent(
		...
		requestworkflow.EventReason(newState, newReason, "BackupRequest"),
		note,
	)
}
