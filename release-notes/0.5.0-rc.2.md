`0.5.0-rc.2` is the first published release candidate for OpenBao Operator
...
migration procedure. RC2 also corrects the release workflow so each image scan
checks out the tagged repository before loading the local Trivy setup action.
...
```sh
kubectl apply -f https://github.com/dc-tec/openbao-operator/releases/download/0.5.0-rc.2/crds.yaml
```
