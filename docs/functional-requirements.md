# Functional Requirements Specification

## 1. Security (TLS Authority)
* **FR-SEC-01:** The Operator MUST generate a self-signed Root CA if one does not exist.
* **FR-SEC-02:** The Operator MUST issue leaf certificates for the OpenBao cluster with correct SANs (internal Kubernetes service/pod DNS names such as `*.security.svc`, `prod-cluster.security.svc`, `localhost`, and any configured IP SANs).
* **FR-SEC-03:** The Operator MUST automatically rotate certificates 7 days (configurable) before expiry.
* **FR-SEC-04:** The Operator MUST trigger a hot-reload signal path when certs are updated by updating pod-level metadata (for example, the `openbao.org/tls-cert-hash` annotation) that a sidecar container can use to send `SIGHUP` to OpenBao locally, without requiring `pods/exec` privileges.

## 2. Infrastructure (Bootstrapping)
* **FR-INF-01:** The Operator MUST create a StatefulSet, Service, and ConfigMap based on the CR.
* **FR-INF-02:** The Operator MUST inject Raft discovery configuration into the ConfigMap that:
  * Uses a deterministic `retry_join` pointing to the pod-0 DNS name during initial bootstrap.
  * Uses a Kubernetes go-discover based `retry_join` with `auto_join` after initialization so that new pods can join based on the `openbao.org/cluster=<name>` label.
* **FR-INF-03:** The cluster MUST bootstrap into a Quorum (Health status 200) without manual intervention, using static auto-unseal managed by the Operator.
* **FR-INF-04:** The Operator MUST automate initial cluster initialization via the OpenBao HTTP API (`PUT /v1/sys/init`) during Day 0 bootstrapping when self-initialization is disabled.
* **FR-INF-05:** The Operator MUST store the initial root token in a per-cluster Kubernetes Secret with appropriate RBAC restrictions when self-initialization is disabled.
* **FR-INF-06:** The Operator MUST use a single-pod bootstrapping strategy (start with 1 replica, initialize, then scale to desired replicas) to avoid split-brain during initialization.
* **FR-INF-07:** The Operator MUST support opt-in OpenBao self-initialization via the `initialize` stanza in `config.hcl`, allowing users to define initial audit devices, auth methods, secret engines, and policies.
* **FR-INF-08:** When self-initialization is enabled, the Operator MUST NOT create or manage a root token Secret, as the root token is automatically revoked by OpenBao after self-init completes.
* **FR-INF-09:** The Operator MUST support providing self-init configuration via `spec.selfInit.requests[]`, allowing users to specify API operations (path, operation, data) to execute during initialization.

## 3. Operations (Upgrades)
* **FR-OPS-01:** The Operator MUST detect a version change in the CRD.
* **FR-OPS-02:** The Operator MUST NEVER update a Leader pod. It must force a step-down first.
* **FR-OPS-03:** The Operator MUST pause the upgrade if the cluster health is degraded.
* **FR-OPS-04:** The Operator MUST provide a maintenance mode via `OpenBaoCluster.Spec.Paused` that stops reconciliation for that cluster while preserving the ability to perform cleanup on delete.
* **FR-OPS-05:** The Operator MUST validate version changes using semantic versioning comparison.
* **FR-OPS-06:** The Operator MUST block version downgrades by default to prevent data corruption.
* **FR-OPS-07:** The Operator SHOULD warn (via `Status.Conditions`) when skipping minor or major versions during an upgrade.
* **FR-OPS-08:** The Operator MUST NOT automatically roll back failed upgrades; instead it MUST halt the upgrade and set `Degraded=True` with a clear reason.
* **FR-OPS-09:** The Operator MUST document manual rollback procedures in the operational guide.
* **FR-OPS-10:** Partial upgrades (mixed versions across pods) MUST be surfaced via `Status.Conditions` with `Reason=PartialUpgrade`.
* **FR-OPS-11:** The Operator MUST persist upgrade progress in `Status.Upgrade` to allow resumption after Operator restart.
* **FR-OPS-12:** The Operator MUST verify quorum health (majority of pods healthy) before and after each pod update during an upgrade.
* **FR-OPS-13:** The Operator MUST enforce configurable timeouts for upgrade operations (step-down, pod ready, health check, Raft sync).
* **FR-OPS-14:** The Operator MUST support optional pre-upgrade backup via `spec.backup.preUpgradeSnapshot`, blocking the upgrade if the backup fails.
* **FR-OPS-15:** The Operator MUST use authenticated API calls to OpenBao for health checks and leader step-down operations.
* **FR-OPS-16:** The Operator MUST wait for Raft log index synchronization after each pod update before proceeding to the next pod.

## 4. Networking & External Access
* **FR-NET-01:** The Operator MUST support configuring external access to OpenBao via Kubernetes `Service` (e.g., LoadBalancer) and/or `Ingress` resources based on the `OpenBaoCluster` spec.
* **FR-NET-02:** The Operator MUST ensure that any externally exposed hostnames are reflected in the TLS SANs (via `ExtraSANs` or equivalent configuration).
* **FR-NET-03:** The Operator MUST document recommended patterns for integrating with Ingress controllers and/or service meshes without breaking mTLS.
* **FR-NET-04:** The Operator MUST support Kubernetes Gateway API as an alternative to Ingress for external access.
* **FR-NET-05:** The Operator MUST support configuring `HTTPRoute` resources when Gateway API is enabled, routing traffic through a referenced `Gateway`.
* **FR-NET-06:** The Operator MUST ensure Gateway API hostnames are reflected in the TLS SANs, consistent with Ingress behavior.

## 5. Disaster Recovery (Backup)
* **FR-DR-01:** The Operator MUST accept a Cron schedule (e.g., "0 3 * * *").
* **FR-DR-02:** The Operator MUST stream the Raft snapshot directly to object storage without buffering to disk.
* **FR-DR-03:** The Operator MUST support configurable backup retention by count (`spec.backup.retention.maxCount`) and/or age (`spec.backup.retention.maxAge`).
* **FR-DR-04:** The Operator MUST delete backups exceeding retention limits after successful new backup upload.
* **FR-DR-05:** The Operator MUST NOT start a new backup if one is already in progress for the same cluster.
* **FR-DR-06:** The Operator SHOULD skip scheduled backups if the cluster is in `Upgrading` phase and set a Condition explaining the skip.
* **FR-DR-07:** The Operator SHOULD verify backup integrity after upload by reading back the object metadata (ETag or checksum where available).
* **FR-DR-08:** The Operator MUST surface backup size and duration in `Status.Backup` and metrics.
* **FR-DR-09:** The Operator MUST use authenticated API calls to OpenBao for snapshot operations.
* **FR-DR-10:** The Operator MUST support multiple object storage authentication methods: static credentials, session tokens, and workload identity (IRSA, GKE Workload Identity).
* **FR-DR-11:** The Operator MUST use predictable, sortable backup naming: `<pathPrefix>/<namespace>/<cluster>/<timestamp>-<uuid>.snap`.
* **FR-DR-12:** The Operator MUST track consecutive backup failures in `Status.Backup.ConsecutiveFailures` and expose via metrics.
* **FR-DR-13:** The Operator MUST calculate and store the next scheduled backup time in `Status.Backup.NextScheduledBackup`.
* **FR-DR-14:** The Operator MUST warn (via `Status.Conditions`) if the backup schedule is more frequent than 1 hour.
* **FR-DR-15:** The Operator MUST reject (via validating webhook) backup schedules more frequent than 15 minutes.
* **FR-DR-16:** For self-init clusters (no root token), users MUST configure a backup-capable token via `spec.selfInit.requests[]`; backups are skipped with `Reason=NoBackupToken` if not configured.
* **FR-DR-17:** The Operator MUST support multipart uploads for snapshots larger than 100MB.

## 6. Multi-Tenancy & Isolation
* **FR-MT-01:** The Operator MUST support managing multiple `OpenBaoCluster` resources in a single Kubernetes cluster.
* **FR-MT-02:** The Operator MUST support multiple `OpenBaoCluster` resources per namespace, with no cross-impact between clusters.
* **FR-MT-03:** A failure or misconfiguration in one `OpenBaoCluster` MUST NOT prevent reconciliation of other `OpenBaoCluster` resources.
* **FR-MT-04:** The Operator MUST scope RBAC so that namespace-scoped users can manage only the `OpenBaoCluster` resources in their namespaces (when desired by cluster admins).
* **FR-MT-05:** The Operator MUST avoid sharing Secrets or ConfigMaps between different `OpenBaoCluster` instances by name; each cluster MUST have uniquely named resources to prevent cross-tenant impact.

## 7. Non-Functional Requirements
* **FR-NFR-01:** Under normal load, the Operator SHOULD reconcile changes to an `OpenBaoCluster` within 30 seconds.
* **FR-NFR-02:** The Operator MUST implement idempotent reconciliation so that re-running the Reconcile loop does not cause unintended side effects.
* **FR-NFR-03:** The Operator MUST implement exponential backoff for transient errors (e.g., OpenBao API unavailable, object storage temporarily unreachable).
* **FR-NFR-04:** The Operator MUST support at least 10 concurrent `OpenBaoCluster` resources per Kubernetes cluster without degradation beyond agreed SLOs.
* **FR-NFR-05:** The Operator Pod resource usage (CPU/memory) SHOULD be bounded and documented for typical cluster sizes (e.g., up to 5 OpenBao nodes per cluster).
* **FR-NFR-06:** The Operator MUST implement rate limiting and backoff at the controller level to prevent hot reconcile loops from overloading the API server in multi-tenant scenarios.

## 8. Observability & Diagnostics
* **FR-OBS-01:** The Operator MUST expose basic metrics (e.g., reconciliation duration, reconciliation failures, number of managed `OpenBaoCluster` resources) via a standard HTTP endpoint suitable for scraping by common monitoring systems.
* **FR-OBS-02:** The Operator MUST emit structured logs including `namespace`, `name` of the `OpenBaoCluster`, and a correlation identifier per reconciliation loop.
* **FR-OBS-03:** The Operator MUST surface critical failures (e.g., upgrade failure, backup failure) via `Status.Conditions` on the `OpenBaoCluster` resource.
* **FR-OBS-04:** The Operator MUST NOT interfere with or override OpenBao's own telemetry configuration; instead it SHOULD allow users to configure OpenBao telemetry via the CRD and rely on OpenBao to expose telemetry to their monitoring system of choice.
* **FR-OBS-05:** The Operator MUST expose metrics sufficient to drive alerts on cluster readiness, upgrade status, backup success/failure, and reconciliation errors.
* **FR-OBS-06:** The Operator MUST expose upgrade-specific metrics: `openbao_upgrade_duration_seconds`, `openbao_upgrade_pod_duration_seconds`, `openbao_upgrade_stepdown_total`, `openbao_upgrade_stepdown_failures_total`.
* **FR-OBS-07:** The Operator MUST expose backup-specific metrics: `openbao_backup_last_duration_seconds`, `openbao_backup_last_size_bytes`, `openbao_backup_success_total`, `openbao_backup_consecutive_failures`.
* **FR-OBS-08:** The Operator MUST expose TLS metrics: `openbao_tls_cert_expiry_timestamp`, `openbao_tls_rotation_total`.
* **FR-OBS-09:** All per-cluster metrics MUST be labeled with at least `namespace` and `name` of the `OpenBaoCluster`.

## 9. Compatibility & Support Matrix
* **FR-COMP-01:** The Operator MUST be cloud-agnostic and not rely on provider-specific APIs for core functionality.
* **FR-COMP-02:** The Operator MUST support generic object storage for backups (e.g., S3-compatible, GCS-compatible, Azure Blob-compatible) via configurable endpoints and credentials.
* **FR-COMP-03:** The Operator MUST document the minimum supported Kubernetes and OpenBao versions and MUST validate unsupported versions with clear error messages in `Status.Conditions`.
* **FR-COMP-04:** The Operator requires **OpenBao v2.4.0 or later** due to the use of static auto-unseal (`seal "static"`). Earlier versions do not support this feature and will fail to start.
