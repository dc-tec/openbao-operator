...
| `GatewayIntegrationReady` | Operator-known Gateway API prerequisites, controller support, and managed Route attachment for `spec.gateway` | `GatewayIntegrationReady`, `GatewayAPIMissing`, `GatewayReferenceMissing`, `GatewayClassMissing`, `GatewayClassPending`, `GatewayClassNotAccepted`, `GatewayVersionUnsupported`, `GatewayFeatureUnsupported`, `GatewayCapabilitiesUnknown`, `GatewayNotProgrammed`, `GatewayProgrammingPending`, `GatewayListenerIncompatible`, `GatewayRoutePending`, `GatewayRouteNotAccepted`, `GatewayRouteReferencesUnresolved`, `Paused` |
